			...
			<view style="display: flex; flex-direction: row; flex-wrap: wrap;">
				<block v-for="(list,index) in dataList" :key="index">
					<view style="width: 32.5%; height: 350upx; background-color: #000000; margin-top: 5upx; margin-left: 5upx;">
						<image @click="toAppVideo(index)" :src="list.src+'?x-oss-process=video/snapshot,t_1000,f_jpg'" mode="aspectFill" style="width: 100%; height: 100%;"></image>
						<!-- <image @click="toAppVideo(index)" :src="list.videoHref" mode="aspectFill" style="width: 100%; height: 100%;"></image> -->
					</view>
				</block>
			</view>
		</scroll-view>
...
<script>
	// import dataList from '../index/data.js'
	export default {
		data() {
			return {
				windowWidth: 0,
		...
		methods: {
			toAppVideo(index){
				...
				uni.navigateTo({
					url: './infoVideo/infoVideo?option='+location+'&page='+this.page+'&index='+index,
