<template>
  <view>
    <u-navbar title="订单详情" :autoBack="true" rightText="联系客户"></u-navbar>
    <view :style="'margin-top:' + CustomBar + 'px;'" class="padding-bottom">
      <fxbStatusCard :status="statusArr[status]" :tips="status==0?'剩余23小时58分':status==1||status==2||status==3||status==4?'剩余23小时58分':status==5||status==6||status==7||status==8?'已发货，等待买家收货':'订单已完成'" icon="daishenhe"></fxbStatusCard>
      ...
      <!--      物流信息-->
      <view class="bg-white padding-tb margin-top-sm" v-if="status == 5||status == 6||status == 7||status == 8||status == 9">
      ...
      <view class="bg-white padding-tb margin-top-sm">
        ...
        <view class="padding-lr-sm">
          ...
          <view class="flex align-center justify-between margin-top-sm padding-right-xs"
                style="padding-bottom: 10rpx"
                v-if="status == 5||status == 6||status == 7||status == 8||status == 9"
          >
