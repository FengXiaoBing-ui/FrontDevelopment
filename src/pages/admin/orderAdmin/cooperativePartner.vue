<template>
  <view class="content">
      ...
      <view class="padding-sm contentBox"
        ...
        <view class="whiteBoxShadow margin-tb-sm" v-if="isAfterSales==false" v-for="item in currentOrderList" :key="item.oddNumber" @click="gotoPaths(item.status)">
          <view class="flex justify-between">
            ...
            <view :class="`${item.status===1||item.status===2||item.status===3?'text-orange':item.status===4?'text-cyan':item.status===9||item.status===8||item.status===5||item.status===6||item.status===7?'text-blue':'text-green'}`">
    ...
    <view class="fixed btn padding-sm bg-white">
      <button class="text-white bg-red">新增订单</button>
    </view>
...
export default{
  ...
  methods: {
    ...
    transStatust(status) {
      const tempArr = ["待审核","待审核","待审核", "未通过","进行中", "进行中","进行中","进行中","进行中", "已完成"];
      return tempArr[status - 1];
    },
    confirm(e) {
      console.log(e);
    ...
    tabSelect(e) {
      ...
      if(this.isAfterSales){
        ...
        this.tempArr = [
          ...
          "item.status === 4",
          "item.status === 5||item.status === 6||item.status === 7||item.status === 8||item.status === 9",
          "item.status === 10"];
      }
    ...
    gotoPath(status) {
      ...
      uni.navigateTo({
        url: "/pages/admin/orderAdmin/cooperationOne?status=" + (status-1)
      });
    },
    gotoPaths(status) {
      // console.log(e);
      ...
      // '/pages/admin/afterSales/afterSalesDetails?status=1?&listStatus=1')"
      uni.navigateTo({
        url: "pages/admin/orderAdmin/cooperationTwo?status=" + (status-1)
      });
    },
  }
}
