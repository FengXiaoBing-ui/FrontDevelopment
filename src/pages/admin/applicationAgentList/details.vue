<template>
    <view class="content">
        <u-navbar :border="false" :fixed="true" title="申请详情" :autoBack="true">
        </u-navbar>
        <view class="padding-bottom"
              :style="'margin-top:'+ CustomBar + 'px;'+ ((status==1||status==0)?'padding-bottom:100rpx':'')">
            <view>
                <fxbStatusCard :status="statusArr[status]" :tips="status==0?'等待商家审核':status==1?'该地区代理已满':status==2?'若48小时内未支付，合同自动作废':'激活合同时间超时'" icon="daishenhe"></fxbStatusCard>
            </view>
<!--            <view class="whiteBoxShadow margin-bottom-sm" v-if="status==1||status==2||status==3">-->
...
<!--                        <view>2022-08-30 12:12:23</view>-->
<!--                    </view>-->
<!--                </template>-->
<!--                <template v-if="status==2">-->
<!--                    <view class="flex justify-between align-center margin-top-sm">-->
<!--                        <view class="text-gray">剩余时间</view>-->
<!--                        <view class="text-red">23小时59分</view>-->
<!--                    </view>-->
<!--                    <view class="margin-top-sm bg-orange light radius padding-tb-sm padding-lr">-->
<!--                        <view class="text-orange text-sm">温馨提示</view>-->
<!--                        <view class="text-orange text-sm margin-top-xs">收到合同后若48小时内未进行签约(付款)，合同自动过期</view>-->
<!--                    </view>-->
<!--                </template>-->
<!--                <view v-if="status==3" class="flex justify-between align-center margin-top-sm">-->
<!--                    <view class="text-gray">过期原因</view>-->
<!--                    <view>激活合同时间超时</view>-->
<!--                </view>-->
<!--            </view>-->
            ...
            <view class="whiteBoxShadow" style="transform: translateY(-20rpx)">
                ...
                <view class="flex justify-between margin-top-sm">
                    ...
                    <view class="flex flex-direction align-end">
                        <view class="text-red text-bold">¥50000.00</view>
                        <view class="text-gray text-sm" style="font-style: oblique">伍万元整</view>
                    </view>
                </view>
                <view class="bg-orange text-sm light radius padding-sm margin-top-sm">
                    <text>温馨提示</text>
                    <view class="margin-top-sm">若您选择发部分货，剩余未发货将存入平台云库存，后续发货请前往云库存进行发货，谢谢！</view>
                </view>
                <button v-if="status==2" @click="gotoPath('/pages/admin/adminContract/bondPay')" style="border-color: red !important;color: red !important;" class="cu-btn margin-top bg-white u-border radius w100">缴纳保证金</button>
            </view>

            <view class="whiteBoxShadow margin-bottom-sm" v-if="status==0||status==1">
                <boxTitle title="申请信息"></boxTitle>
                <view class="flex justify-between align-center margin-bottom-sm">
                    <text class="text-gray">申请代理编号</text>
                    <text>12312332432454</text>
                </view>
                <view class="flex justify-between align-center margin-bottom-sm">
                    <text class="text-gray">代理政策名称</text>
                    <text>东方国宾·尊致代理政策</text>
                </view>
                <view class="flex justify-between align-center margin-bottom-sm">
                ...
                <view class="flex justify-between align-center margin-bottom-sm">
                    <text class="text-gray">代理区域</text>
                    <text>贵州省贵阳市观山湖区世纪城街道</text>
                </view>
                <view class="flex justify-between align-center margin-bottom-sm">
                    <text class="text-gray">申请单位</text>
<!--                    <text class="flex align-center">贵阳市万隆烟酒行 <text class="bg-orange light padding-xs radius text-xs margin-left-xs">续签</text> </text>-->
                    <text>贵阳市万隆烟酒行</text>
                </view>
                <view class="flex justify-between align-center margin-bottom-sm"
                      @click="gotoPath('/pages/admin/adminContract/agentBusinessLicense')">
                    <text class="text-gray">营业执照信息</text>
                    <text class="cuIcon-right margin-left text-gray" style="font-size: 30rpx"></text>
                </view>
                <view class="flex justify-between align-center margin-bottom-sm"
                      @click="gotoPath('/pages/admin/adminContract/agencyIDInfo')">
            ...
            <template v-if="status==2||status==3">
                <view class="whiteBoxShadow margin-bottom-sm">
                    ...
                    <u-read-more ref="uReadMore" textIndent="0" :toggle="true" showHeight="220" color="gray" openText="向上收起"
                                 closeText="查看更多">
                        <view class="flex justify-between align-center margin-bottom-sm">
                            <text class="text-gray">申请代理编号</text>
                ...
                <view class="whiteBoxShadow margin-bottom-sm">
                    <view class="flex justify-between align-center" @click="gotoPdf">
                        <text>查看合同</text>
                ...
                <view class="whiteBoxShadow margin-bottom-sm">
                    <boxTitle title="政策明细"></boxTitle>
                    <view class="margin-bottom-sm text-bold">代理价格</view>
...
<!--            <view class="whiteBoxShadow margin-bottom-sm">-->
<!--                <boxTitle :title="status==0||status==1?'申请商品':'合同商品'"></boxTitle>-->
<!--                <twoGoodsInfoCard-->
<!--                    :img-url="(status==0||status==1)?'/productDetails/Slice%201.png':null"-->
<!--                    goodsName="东方国宾53°瓶装酱香型 500ml"-->
<!--                    goodsType="酱香型"-->
<!--                    goodsVolume="500ml"-->
<!--                    goodsDegrees="53°"-->
<!--                    retailPrice="365.00"-->
<!--                    :agentPrice="(status==2||status==3)?'2400':null"-->
<!--                    namePrice="代理价："-->
<!--                >-->
<!--                </twoGoodsInfoCard>-->
...
<!--            </view>-->
            ...
            <view>
              <view class="whiteBoxShadow margin-bottom-sm" v-if="status==0||status==1">
                ...
                <view>
                  <view class="flex align-center padding-lr-sm margin-bottom">
                    <view>
                      <image src="../returnExchange/picture/alcohol.png"
                             style="width: 135rpx;height: 135rpx;border-radius: 20rpx"></image>
                    </view>
                    <view class="w100 margin-left">
                      <view class="text-bold text-lg margin-bottom-xs">东方国宾·尊致酒53°瓶装500ml</view>
            ...
            <view v-if="status==0||status==1" class="whiteBoxShadow margin-bottom-sm">
                <boxTitle title="政策明细"></boxTitle>
                <u-read-more ref="uReadMore" textIndent="0" showHeight="600" :toggle="true" color="gray" openText="向上收起"
                             closeText="查看更多">
                    <view class="text-bold">东方国宾酒代理政策1（区级）</view>
                    <view class="flex justify-between margin-top-sm">
                        <view class="text-gray">代理商品</view>
                        <view class="flex text-black flex-direction align-end">东方国宾·尊致500ml瓶装</view>
                    </view>
                    <view class="flex justify-between margin-top-sm">
                        <view class="text-gray">代理地区</view>
                        <view class="flex text-black flex-direction align-end">贵州省贵阳市观山湖区世纪城街道</view>
                    </view>
                    <view class="flex justify-between margin-top-sm">
                        <view class="text-gray">代理等级</view>
                        <view class="flex text-black flex-direction align-end">乡镇/街道级</view>
                    </view>
                    ...
                    <view class="text-bold padding-top margin-top u-border-top">首单进货</view>
                    <view class="flex justify-between margin-top-sm">
                        <view class="text-gray">首单进货商品总量</view>
                        <view class="flex flex-direction align-end">
                            <view>600瓶</view>
                            <view class="text-gray text-sm" style="font-style: oblique">100件</view>
                        </view>
            ...
            <view>
              <view class="whiteBoxShadow margin-bottom-sm" v-if="status==2||status==3">
            ...
            <view class="whiteBoxShadow margin-bottom-sm">
                <boxTitle title="首单支付金额" :right-text="status==3?'已过期':''" right-text-class="text-gray"></boxTitle>
                <view class="flex justify-between margin-top-sm">
                    <view class="text-gray">首单进价金额</view>
                    <view class="flex flex-direction align-end">
                        <view class="text-red text-bold">¥155400.00</view>
                        <view class="text-gray text-sm" style="font-style: oblique">拾伍万伍仟肆佰元整</view>
                    </view>
                </view>
                <button v-if="status==2" @click="gotoPath('/pages/admin/applicationAgentList/payContractMoney')" style="border-color: red !important;color: red !important;" class="cu-btn margin-top bg-white u-border radius w100">去支付</button>
            </view>
...
export default {
    name: "applicationAgentList-details",
    components: {
        boxTitle,
        fxbTable,
        twoGoodsInfoCard,
    ...
    methods: {
        gotoPay() {
            ...
            uni.navigateTo({
                url: "/pages/admin/applicationAgentList/payContractMoney"
            });
        },
        gotoPath(path) {
            uni.navigateTo({
