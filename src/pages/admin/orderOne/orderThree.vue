<template>
  <view>
    ...
    <view :style="'margin-top:' + CustomBar + 'px;'" class="padding-bottom">
      ...
      <fxbStatusCard :status="statusArr[status]" :tips="status==0||status==1||status==2?'剩余23小时58分':status==3?'商家已驳回您的售后申请':status==4||status==5||status==6||status==7||status==8||status==9||status==10?'您已同意售后，正在处理':'您的售后已处理完成'" icon="daishenhe"></fxbStatusCard>
      <view style="transform: translateY(-20rpx)">
        ...
        <view class="bg-white padding-tb" style="border-radius: 10rpx 10rpx 0 0" v-if="status==4||status==5||status==6||status==8||status==9||status==7||status==10||status==11||status==12">
          ...
          <u-steps current="1" dot inactiveColor="#AFAFAF" activeColor="#F37B1D">
            ...
            <u-steps-item title="商家退款" v-if="status==10||status==12||status==6||status==8||status==9||status==7"></u-steps-item>
            <u-steps-item title="银行处理"></u-steps-item>
            <u-steps-item title="退款成功"></u-steps-item>
          </u-steps>
          <view class="bg-white padding-tb" style="border-radius: 10rpx 10rpx 0 0" v-if="status==6">
            <view class="text-red padding-lr-sm">您的商品经查看暂无质量问题，因此驳回您的售后申请</view>
          </view>
          <view class="padding-lr-sm padding-tb" v-if="status==11||status==12||status==9||status==10">
            <button class="bg-red text-white adjustment" @click="toChange()">查看钱款去向</button>
          </view>
          <view class="padding-lr-sm">
            <view v-if="status==7||status==8||status==9||status==10">
        ...
        <view class="bg-white padding-tb" :style="status==0?'border-radius: 10rpx 10rpx 0 0':''" :class="status==0?'':'margin-top-sm'">
          ...
          <view class="padding-lr-sm">
            ...
            <view class="flex align-center justify-between margin-top-sm" style="padding-bottom: 10rpx">
              ...
              <view class="">{{status==2?"换货":status==0||status==3||status==4||status==5||status==11||status==12?"退款":"退款退货"}}</view>
            </view>
            <view class="flex align-center justify-between margin-top-sm" style="padding-bottom: 10rpx">
      ...
      <view class="bg-white padding-tb">
        <view class="flex align-center margin-bottom" style="padding-bottom: 15rpx">
          <view class="cuIcon-titles text-red" style="font-size: 40rpx"></view>
          <view class="text-bold text-lg">退款商品</view>
        </view>
        <view class="padding-lr-sm">
...
export default {
  ...
  onLoad(options) {
    ...
    if(options.status==4||options.status==5||options.status==11){
      this.title = '商家同意'
    }else {
      this.title = '预约取件'
    }
