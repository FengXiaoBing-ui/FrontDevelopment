  ...
  <view class="content">
    <u-navbar :border="false" :fixed="true"
      ...
      <view slot="left" class="u-nav-slot">
        ...
        <text @click="switchOrder" :class="isAfterSales?'text-black text-xl':'text-gray text-lg'"
              class="margin-left-sm">订单管理
        </text>
        <text @click="switchOrder" :class="isAfterSales?'text-gray text-lg':'text-black text-xl'"
              class="margin-left-sm">售后
        </text>
      </view>
    ...
    <view>
      <view class="padding-sm contentBox"
            :style="{ '--CustomBar': CustomBar + 'px', '--StatusBar': StatusBar + 'px' }" style="padding-bottom: 150rpx">
        ...
        <view class="whiteBoxShadow margin-tb-sm" v-if="isAfterSales==true" v-for="item in currentOrderList" :key="item.oddNumbers+tempArr.length" @click="gotoPath(item.status)">
          <view class="flex justify-between">
            ...
            <view :class="`${item.status === 1||item.status === 2||item.status === 3?'text-orange':item.status === 6||item.status === 7||item.status === 4||item.status === 5?'text-cyan':item.status === 10||item.status === 11||item.status === 8||item.status === 9?'text-blue':'text-green'}`">
        ...
        <view class="whiteBoxShadow margin-tb-sm" v-if="isAfterSales==false" v-for="item in currentOrderList" :key="item.oddNumber" @click="gotoPaths(item.status)">
          <view class="flex justify-between">
            ...
            <view :class="`${item.status===1||item.status===2||item.status===3?'text-orange':item.status===4?'text-cyan':item.status === 5||item.status === 6||item.status === 7||item.status === 8||item.status === 9||item.status === 10||item.status === 11?'text-blue':'text-green'}`">
    ...
    <view class="fixed btn padding-sm bg-white">
      <button class="text-white bg-red" @click="goToItem()">新增订单</button>
    </view>
...
export default{
  ...
  data(){
    return{
      ...
      orderList: [
        ...
        {
          status: 13,
          oddNumbers: 2309262524,
          oddNumber:5251515616,
          goodsShipped: "东方国宾·尊致酒酱香型白酒",
          shipmentQuantity: 2,
          agencyPrice: 259.00
        },
  ...
  methods: {
    ...
    transStatus(status) {
      const tempArr = ["待付款","待付款","待付款", "待发货","待发货", "待发货","待发货","待收货","待收货","待收货","待收货", "已完成", "已完成"];
      return tempArr[status - 1];
    },
    transStatust(status) {
      const tempArr = ["待审核","待审核","待审核", "未通过","进行中", "进行中","进行中","进行中","进行中","进行中","进行中", "已完成", "已完成"];
      return tempArr[status - 1];
    },
    confirm(e) {
      console.log(e);
    ...
    tabSelect(e) {
      ...
      if(this.isAfterSales){
        this.tempArr = [
          ...
          "item.status === 6||item.status === 7||item.status === 4||item.status === 5",
          "item.status === 10||item.status === 11||item.status === 8||item.status === 9",
        ...
        this.tempArr = [
          ...
          "item.status === 1||item.status === 2||item.status === 3",
          "item.status === 4",
          "item.status === 5||item.status === 6||item.status === 7||item.status === 8||item.status === 9||item.status === 10||item.status === 11",
          "item.status === 12||item.status === 13"];
      }
    ...
    gotoPath(status) {
      ...
      uni.navigateTo({
        url: "/pages/admin/orderOne/orderTwo?status=" + (status-1)
      });
    },
    gotoPaths(status) {
      // console.log(e);
      ...
      // '/pages/admin/afterSales/afterSalesDetails?status=1?&listStatus=1')"
      uni.navigateTo({
        url: "pages/admin/orderOne/orderThree?status=" + (status-1)
      });
    },
    goToItem(){
      uni.navigateTo({
        url: "pages/admin/orderOne/selectItem"
      });
    }
  }
}
