    ...
    <view class="content">
        ...
        <u-navbar :border="false" :fixed="true" :autoBack="true" @rightClick="switchOrder"
            ...
            <view slot="center" class="u-nav-slot">
                <view class="text-lg">{{ isAfterSales ? "售后列表" : "样品订单" }}</view>
            ...
            <view slot="right" class="u-nav-slot">
                <text>{{ isAfterSales ? "样品订单" : "售后列表" }}</text>
            </view>
        </u-navbar>
        ...
        <view class="padding-sm contentBox"
            ...
            <view class="whiteBoxShadow margin-bottom-sm" v-for="(item,index) in currentOrderList" :key="index">
                <view class="flex justify-between">
                    <view class="flex align-center">
                        <view class="cuIcon-titles text-red"></view>
                        <view class="text-bold">退换货编号：{{ item.oddNumbers }}</view>
                    </view>
                    <!--                    <text class="text-gray">订单单号：-->
                    <!--                        <text class="text-black">{{ item.oddNumbers }}</text>-->
                    <!--                    </text>-->
        ...
        <u-popup :show="show" mode="right" @close="close" @open="open">
            <view class="scrollViewBox"
                ...
                <view class="flex align-center justify-around">
                    <button style="width: 45%" class="cu-btn u-border bg-white radius margin-right-sm" @click="close">
                        重置
                    </button>
...
export default {
    ...
    watch: {
        isAfterSales() {
            this.currentOrderList = this.isAfterSales ? this.afterList : this.orderList;
            this.tabIndex = 0;
...
// {{ (item.status == 0 || item.status == 1) ? "待付款" : item.status == 2 ? "待发货" : item.status == 3 ? "待收货" :item.status == 4? "已完成":""
// }}
        transStatus(status) {
        ...
        //     uni.navigateTo({
        //         url: "/pages/admin/afterSales/afterSalesDetails?status=" + status
        //     });
        // },
        gotoDetail(status, statusList, state) {
            if (this.isAfterSales) {
                uni.navigateTo({
                    url: "/pages/admin/afterSales/afterSalesDetails?status=" + status + "&statusList=" + statusList + "&state=" + state + "&condition=0"
                });
            } else {
        ...
        tabSelect(e) {
            ...
            if (this.isAfterSales) {
                ...
                this.currentOrderList = this.afterList.filter(item => {
