<template>
  <view class="content">
    <u-navbar :border="false" :fixed="true" @rightClick="switchOrder"
      ...
      <view slot="left" class="u-nav-slot text-gray">
        ...
        <text @click="switchCrowdFunding" :class="isCrowdFunding?'text-black text-xl text-bold':'text-gray text-lg'"
              class="margin-left-sm text-lg">条码众筹
        </text>
        <text @click="switchCrowdFunding" :class="isCrowdFunding?'text-gray text-lg':'text-black text-xl text-bold'"
              class="margin-left-sm text-lg">条码招标
        </text>
      </view>
      <view slot="right">
        <view class="text-orange text-lg">招商须知</view>
      </view>
    </u-navbar>

    <view :style="'margin-top:' + CustomBar +  'px'">
      <view style="width:750rpx" class="search padding-bottom-xs bg-white">
        <!--产品列表导航-->
        <view class="padding-lr" style="padding-top: 14rpx">
          <view class="flex align-center bg-gray" style="border-radius: 4px">
    ...
    <view>
      ...
      <view class="product margin-top-sm">
        <view class="flex flex-wrap justify-between padding-lr-sm">
          <view @click="jumpDetails(item.status,item.state,item.show)" v-for="(item,index) in productList"
                :key="index"
            ...
            <view class="flex align-start">
              <view class="basis-xs">
                <text
                    class="padding-xs bg-blue" style="border-radius: 10rpx 10rpx 0 10rpx">
            ...
            <view v-if="isCrowdFunding" class="margin-top-sm">
      ...
      <u-popup :show="shows" mode="center"  @close="close" @open="open" :closeable="true">
        <view class="padding" style="width: 630rpx">
          <view class="text-xl text-bold text-center margin-top">招商须知</view>
          <view style="line-height: 48rpx;text-indent: 1rem" class="margin-top">   条码招商是平台新推出的一款营收功能，用户招商成功后平台帮助卖酒，收益将按购买股份多少分配给投资者，投资者不用花费时间经营平台交易后直接分配给投资者；</view>
          <view class="text-orange text-lg text-bold margin-tb">什么是众筹：</view>
          <view style="line-height: 48rpx;text-indent: 1rem">   众筹是投资者按最低购买量直接购买股权，不需缴纳保证金，众筹金额达到后签署合同，平台根据销售金额按比例进行分配；众筹特点是投资金额少，相应利润少；</view>
          <view class="text-orange text-lg text-bold margin-tb">什么是招投标：</view>
          <view style="line-height: 48rpx;text-indent: 1rem" class="margin-bottom">   招投标需缴纳保证金，并有人数限制和相应的资质审核，报名后缴纳保证金、上传资质文件审核通过并中标后缴纳合作费用；招投标特点投资大，收益高；</view>
        </view>
      </u-popup>
    </view>
  </view>
...
export default {
  ...
  onLoad() {
    this.productList = this.orderList;
  },
  methods: {
    switchCrowdFunding() {
    ...
    jumpDetails(status, state, show) {
      uni.navigateTo({
        url: `pages/admin/tender/detailsList?status=${status}&state=${state}&show=${show}&statusText=${this.activityList[status].name}&isCrowdFunding=${this.isCrowdFunding}`
      });
    }
  }
};
