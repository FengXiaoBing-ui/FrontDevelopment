  ...
  <view class="content" style="padding-bottom: 100rpx">
    ...
    <view class="bg-white margin-top-sm padding-sm">
      <view class="flex justify-between align-center">
        ...
        <view class="text-blue">{{ isCrowdFunding?'众筹中':'招标中' }}</view>
      </view>
      ...
      <view class="margin-top-sm" v-if="isCrowdFunding">
        ...
        <view class="padding-xs margin-top-sm" style="background-color: rgba(229,77,66,0.05)">
...
<!--      <winning v-show="activeText==='中标公告'"></winning>-->
    ...
    <view v-if="status==1" class="fixed bg-white padding-xs w100">
      <button class="cu-btn bg-red radius w100" @click="goToWant()">{{isCrowdFunding?'我要众筹':'我要投标'}}</button>
    </view>

    <view>
      <u-popup :show="show" @close="close" @open="open" mode="center">
        <view class="text-center padding-tb padding-lr-sm" style="width: 510rpx">
          <view class="cuIcon-infofill text-orange" style="font-size: 68rpx"></view>
          <view class="text-bold text-lg margin-top">确认投标</view>
          <view class="text-gray margin-top-sm">您已确认参与该次投标？</view>
        </view>
        <view class="flex align-center justify-between margin-top-sm">
          <view class="w50 cu-btn" @click="close">取消</view>
          <view class="w50 bg-red cu-btn" @click="close">确定</view>
        </view>
      </u-popup>
    </view>
  </view>
...
export default {
  ...
  methods:{
    ...
    goToWant(){
      if(this.isCrowdFunding){
        uni.navigateTo({
          url: "/pages/admin/tender/wantCrowdfunding"
        });
      }else {
        this.show = true;
      }
