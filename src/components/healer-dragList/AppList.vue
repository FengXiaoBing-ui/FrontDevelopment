<template>
    ...
    <view class="">
        ...
        <movable-area class="movarea" ref="areaBox" id="areaBox">
            ...
            <view class="appList">
                <view class="padding-sm app-li flex flex-direction align-center"
                    ...
                    <view v-if="appItem.tag" :class="'bg-'+appItem.tagColor" class="tag text-xs">
                        {{ appItem.tag }}
                    </view>
                    <!--					<text :class="['appIcon',appItem.appIcon]"></text>-->
                    <view style="width: 50rpx;height: 50rpx" class="t-icon" :class="'t-icon-'+appItem.icon"></view>
                    <text class="appName margin-top-xs text-cut">{{ appItem.appName }}</text>
                    <text class="appicon cuIcon-roundclosefill" :class="isSetting?'':'hide'"
                          @click="deleteAppItem(index)"></text>
            ...
            <!-- 滑块 -->
            <movable-view v-if="moveShow" :animation="false" class="moveV flex flex-direction align-center" :x="moveX"
                          :y="moveY" direction="all"
                          :style="{ width: '32%', height: 160 + 'rpx' }">
                <!--				<text :class="['appIcon',touchItem.appIcon]"></text>-->
                <view style="width: 50rpx;height: 50rpx" class="t-icon" :class="'t-icon-'+touchItem.icon"></view>
...
export default {
    ...
    methods: {
        ...
        gotoPath() {
            ...
            if (this.touchItem.isCharge) {
                uni.navigateTo({
                    url: "/pages/admin/functionCharge?appName=" + this.touchItem.appName
                });
                return false;
            }
            console.log('1857485',this.touchItem.appLink);
            uni.navigateTo({
                url: this.touchItem.appLink
            });
        }
    }
};
