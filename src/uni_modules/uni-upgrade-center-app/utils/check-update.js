...
export function checkUpdate() {
    return new Promise(async (resolve, reject) => {
        ...
        if (code > 0) {
            ...
            if (fileList[0].tempFileURL) res.result.url = fileList[0].tempFileURL;
            // 静默更新，只有wgt有
            // if (is_silently) {
            //     uni.downloadFile({
            //         url: e.result.url,
             ...
             */
            uni.setStorageSync(PACKAGE_INFO_KEY, res.result);
    ...
    .catch(err => {
        if (err.message === "请在App中使用") return uni.showToast({ title: err.message, icon: "none" });
        return err;
    });
}
